...
public class HTTPHeader {
    ...
    public static String getValue(LineNumberReader reader, String name) {
    ...
    public static String getValue(String data, String name) {
        /* Thanks for Stephan Mehlhase (2010-10-26) */
        StringReader strReader = new StringReader(data);
        LineNumberReader lineReader = new LineNumberReader(strReader, Math.min(data.length(), 1024));
        return getValue(lineReader, name);
    }

    public static String getValue(byte[] data, String name) {
        return getValue(new String(data), name);
    }

    public static int getIntegerValue(String data, String name) {
        try {
    ...
    public static int getIntegerValue(byte[] data, String name) {
        try {
            return Integer.parseInt(getValue(data, name));
        } catch (Exception e) {
            return 0;
