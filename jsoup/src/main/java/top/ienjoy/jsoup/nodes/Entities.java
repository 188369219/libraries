...
import static top.ienjoy.jsoup.nodes.Entities.EscapeMode.base;
import static top.ienjoy.jsoup.nodes.Entities.EscapeMode.extended;
...
import top.ienjoy.jsoup.SerializationException;
import top.ienjoy.jsoup.helper.Validate;
import top.ienjoy.jsoup.internal.StringUtil;
import top.ienjoy.jsoup.nodes.Document.OutputSettings;
import top.ienjoy.jsoup.parser.CharacterReader;
...
public class Entities {
        ...
        for (int offset = 0; offset < length; offset += Character.charCount(codePoint)) {
            ...
            if (codePoint < Character.MIN_SUPPLEMENTARY_CODE_POINT) {
                ...
                switch (c) {
                    ...
                    case 0x9, 0xA, 0xD -> accum.append(c);
                    default -> {
    ...
    private static boolean canEncode(final CoreCharset charset, final char c, final CharsetEncoder fallback) {
        return switch (charset) {
            case ascii -> c < 0x80;
            case utf ->
                    true; // real is:!(Character.isLowSurrogate(c) || Character.isHighSurrogate(c)); - but already check above
            default -> fallback.canEncode(c);
        };
    }
