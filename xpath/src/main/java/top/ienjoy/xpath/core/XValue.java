...
public class XValue implements Comparable<XValue> {
    ...
    public Date asDate() {
        if (value instanceof String) {
            try {
                return DateFormatUtils.ISO_8601_EXTENDED_DATETIME_TIME_ZONE_FORMAT.parse((String) value);
    ...
    public Long asLong() {
        if (value instanceof String) {
            //对于带小数点的，四舍五入
            return new BigDecimal((String) value).setScale(0, RoundingMode.HALF_UP).longValue();
        } else if (value instanceof Number) {
