...
import top.ienjoy.xpath.antlr.XpathBaseVisitor;
import top.ienjoy.xpath.antlr.XpathParser;
import top.ienjoy.xpath.exception.XpathMergeValueException;
...
import static top.ienjoy.xpath.antlr.XpathParser.*;
...
public class XpathProcessor extends XpathBaseVisitor<XValue> {
    ...
    public XValue visitRelationalExpr(XpathParser.RelationalExprContext ctx) {
        ...
        if (additiveExprContexts.size() == 1){
            ...
            return switch (ctx.op.getType()) {
                case MORE_ -> XValue.create(left.compareTo(right) > 0);
                case GE -> XValue.create(left.compareTo(right) >= 0);
                case LESS -> XValue.create(left.compareTo(right) < 0);
                case LE -> XValue.create(left.compareTo(right) <= 0);
                case START_WITH -> XValue.create(left.asString().startsWith(right.asString()));
                case END_WITH -> XValue.create(left.asString().endsWith(right.asString()));
                case CONTAIN_WITH -> XValue.create(left.asString().contains(right.asString()));
                case REGEXP_WITH -> XValue.create(left.asString().matches(right.asString()));
                case REGEXP_NOT_WITH -> XValue.create(!left.asString().matches(right.asString()));
                default -> throw new XpathParserException("unknown operator" + ctx.op.getText());
            };
        }else {
            throw new XpathParserException("error equalityExpr near:"+ctx.getText());
        }
    ...
    public XValue visitMultiplicativeExpr(XpathParser.MultiplicativeExprContext ctx) {
        if (ctx.multiplicativeExpr() == null||ctx.multiplicativeExpr().isEmpty()){
            ...
            return switch (ctx.op.getType()) {
                case MUL -> XValue.create(left.asDouble() * right.asDouble());
                case DIVISION -> XValue.create(left.asDouble() / right.asDouble());
                case MODULO -> XValue.create(left.asDouble() % right.asDouble());
                default -> throw new XpathParserException("syntax error, " + ctx.getText());
            };
        }
    }
